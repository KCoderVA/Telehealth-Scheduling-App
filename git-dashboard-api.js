...
class GitActivityAPI {
  ...
  detectRepoPath() {
    ...
    const repoDir = currentPath.replace('/git-activity-dashboard.html', '');
    return repoDir;
  }

  async executeGitCommand(startDate, endDate) {
    try {
      // Create PowerShell command to execute our script
      const command = `powershell.exe -ExecutionPolicy Bypass -File "${this.repoPath}\\Get-GitActivity.ps1" -StartDate "${startDate}" -EndDate "${endDate}" -OutputFormat "json"`;
  ...
  showExecutionStatus(startDate, endDate) {
    ...
    statusDiv.innerHTML = `
            ...
            1. Open PowerShell in this directory<br>
